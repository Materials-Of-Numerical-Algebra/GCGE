...
int TestEigenSolver(void *A, void *B, int argc, char *argv[], struct OPS_ *ops) 
{
	...
	int    nevMax = 75, multiMax = 75, block_size = 20;
	...
	int length_dbl_ws = 2*sizeV*sizeV+10*sizeV
		+(nevMax+multiMax+2*block_size)
		+(nevMax+multiMax)*block_size;
	...
	char   safe_min = 's';
	double compRR_tol            = 2*dlamch(&safe_min);
